...
// yang ditanam tetap ikut. Kalau fotonya beredar sendirian, asalnya tetap terbaca.
//
// Ditaruh di pojok kiri atas dan dibuat sekecil mungkin yang masih terbaca, karena
// isi foto di aplikasi ini justru benda-benda kecil di tepi bingkai: ujung ramp,
// guiding block, pegangan tangga. Menutupinya sama saja merusak buktinya sendiri.
const WM_TEKS = 'landai'
const WM_HIJAU = '#0F6E56'

// Ukuran tanda mengikuti tinggi gambar, bukan angka tetap, supaya proporsinya sama
// pada foto potret maupun lanskap. Dibatasi bawah supaya tetap terbaca pada gambar
// kecil, dan dibatasi atas supaya tidak jadi papan iklan pada gambar besar.
function ukuranTanda(tinggi: number) {
  return Math.round(Math.min(34, Math.max(20, tinggi * 0.042)))
}

// Jalur anak tangga yang melandai, disalin PERSIS dari public/tanda.svg, termasuk
// kurva kuadratik di tiap sudut dalamnya. Ditulis sebagai satu untai supaya kalau
// tandanya berubah, hanya ada satu tempat yang perlu disamakan, dan pengujian bisa
// membandingkan keduanya sebagai teks.
//
// Tidak memuat berkas SVG-nya lewat jaringan: penandaan ini terjadi di lapangan,
// sering pada jaringan buruk, dan satu permintaan tambahan yang gagal akan membuat
// foto batal ditandai tanpa sebab yang bisa dijelaskan ke penggunanya.
...
/**
 * Menanam tanda landai di pojok kiri atas kanvas yang sudah berisi gambar.
 *
 * Bentuknya pil putih, bukan tulisan putih polos. Tulisan polos hilang di atas
 * langit terang dan dinding putih, dua latar yang justru paling sering ada di foto
 * fasilitas umum. Pil putih memberi kontrasnya sendiri, apa pun isi fotonya.
 */
export function tanamTanda(ctx: CanvasRenderingContext2D, lebar: number, tinggi: number) {
  ...
  // Gambar yang terlalu kecil tidak diberi tanda sama sekali. Menaruh pil yang
  // memakan seperempat bingkai bukan penandaan, melainkan perusakan.
...
export async function kecilkanFoto(berkas: File): Promise<Blob> {
  // Menunggu fontnya siap sebelum menggambar. Kalau Public Sans belum termuat, kanvas
  // diam-diam jatuh ke font bawaan sistem, dan tandanya jadi berbeda bentuk antara
  // foto pertama dan foto berikutnya pada perangkat yang sama.
  ...
  // Ditanam setelah gambar, sebelum dikemas jadi berkas. Kegagalan menanam tanda
  // tidak boleh membatalkan fotonya: bukti kondisi lapangan lebih penting daripada
  // penandaan mereknya.
  try {
